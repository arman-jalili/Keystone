...
# For GitHub (gh)
gh auth status
gh repo view arman-jalili/Keystone

# For GitLab (glab)
glab auth status
glab repo view arman-jalili/Keystone
```
...
```bash
# Create milestone
gh api repos/arman-jalili/Keystone/milestones \
  -f title="[EPIC_NAME]" \
  -f description="[EPIC_DESCRIPTION]" \
  -f state="open"

# Get milestone number
gh api repos/arman-jalili/Keystone/milestones --jq '.[] | select(.title=="[EPIC_NAME]") | .number'
```
...
### Repository: arman-jalili/Keystone
### Tool: gh
...
| Repository not found | Check `arman-jalili/Keystone` format (owner/repo) |
