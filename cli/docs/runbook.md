...
The keystone CLI orchestrator is a lightweight Go binary that runs inside CI pipelines. It parses OpenAPI 3.x specs, diffs against cached previous versions, produces a breaking-change verdict, and uploads results to the Keystone server for audit persistence.
...
The CLI process is single-shot — it runs to completion and exits. No graceful shutdown is needed. The async upload goroutine has a 10-second timeout and will complete (or fail) before the process exits.
...
### Incident: CI pipeline failure with exit code 3
1. Check stderr for `SpecParseError` message
2. Verify spec file exists and conforms to OpenAPI 3.x
3. Re-run the pipeline after fixing the spec

### Incident: Breaking changes not detected
1. Verify cache is working — check for `CacheMissError` in output
2. Clear cache and re-run
3. If still failing, escalate to Keystone team

### Incident: Upload failures flooding server
1. Check network connectivity between CI and server
2. Verify API token is valid
3. Upload errors are non-terminal — CLI exit code is unaffected

## Escalation

| Severity | Contact | Response Time |
|----------|---------|---------------|
| Critical | Keystone team on-call | < 30 min |
| High | Keystone team lead | < 2 hours |
| Low | File GitHub issue | < 1 business day |
