...
import com.keystone.breaking.application.dto.ErrorResponse;
...
public class BreakingAnalysisExceptionHandler {
    ...
    @ExceptionHandler(VersionResolutionException.class)
    public ResponseEntity<ErrorResponse> handleVersionResolutionError(VersionResolutionException ex) {
        return ResponseEntity.badRequest()
                .body(new ErrorResponse("VERSION_RESOLUTION_ERROR",
                        "Could not resolve base version for " + ex.getRepository()
                        + "/" + ex.getSpecPath() + ": " + ex.getReason()));
    }
