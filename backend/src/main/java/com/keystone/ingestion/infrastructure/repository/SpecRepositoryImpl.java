...
public class SpecRepositoryImpl implements SpecRepository {
    ...
    public SpecRepositoryImpl(SpringDataSpecRepository specRepo,
                              SpringDataSpecVersionRepository versionRepo) {
    ...
    public OpenApiSpec save(OpenApiSpec spec) {
        var entity = new OpenApiSpecEntity(spec.getId(), spec.getRepository(),
                spec.getSpecPath(), spec.getIngestedAt());
    ...
    public List<SpecVersion> findVersionsBySpecId(UUID specId, int pageSize) {
        return versionRepo.findBySpecIdOrderByIngestedAtDesc(specId, PageRequest.of(0, pageSize))
                .stream()
                .map(this::toDomain)
                .toList();
    }
    ...
    public SpecVersion saveVersion(SpecVersion version) {
        var entity = new SpecVersionEntity(version.getId(), version.getSpecId(),
                version.getCommitSha(), version.getChecksum(),
                version.getRawContent(), version.getIngestedAt());
    ...
    private SpecVersion toDomain(SpecVersionEntity e) {
        return new SpecVersion(e.getId(), e.getSpecId(), e.getCommitSha(),
                e.getChecksum(), e.getRawContent(), e.getIngestedAt());
    }
}
