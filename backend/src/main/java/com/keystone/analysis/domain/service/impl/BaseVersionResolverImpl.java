...
public class BaseVersionResolverImpl implements BaseVersionResolver {
    ...
    @Override
    public BaseVersion resolve(String repository, String specPath,
        ...
        if (explicitBaseRef != null && !explicitBaseRef.isBlank()) {
            log.info("Layer 1: Using explicit base ref '{}' for {}/{}",
                    explicitBaseRef, repository, specPath);
            return new BaseVersion(
                    explicitBaseRef,
                    "Explicit base reference provided by caller",
                    explicitBaseRef,
                    resolvedAt
            );
        }
